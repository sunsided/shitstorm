...
namespace sound {
	...
	SoundEmitter::~SoundEmitter(void)
	{
		// TODO: Was passiert, wenn jemand den Emitter l�scht, ohne dass er deregistriert wurde?
		detachBuffer();
	}
	...
	//! Entfernt den verkn�pften Puffer
	void SoundEmitter::detachBuffer() {
		if (!attachedBuffer) return;
		stop();
		alSourcei(sourceId, AL_BUFFER, 0);

		// Wenn es sich um einen Streaming-Puffer handelt, m�ssen wir uns abmelden
	...
	bool SoundEmitter::callInitEventIfExists() {
		...
		try {
			...
			std::wcerr << "Fehler beim Ausf�hren des Events 'OnInitSoundEmitter': " << e.Message() << std::endl;
		}
		catch(...) {
			std::wcerr << "Fehler beim Ausf�hren des Events 'OnInitSoundEmitter'." << std::endl;
		}
		return true;
	}
